...
@section('title', 'Generate Fill in the Blanks - LecToQuiz')

@section('content')
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-10">
            <h1 class="text-4xl font-bold text-gray-900 mb-3">Fill in the Blanks</h1>
            <p class="text-lg text-gray-600">Create knowledge-testing blanks with hints</p>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-xl p-8">
            <form id="fbForm" action="{{ route('fillblanks.generate') }}" method="POST">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="topic_name" name="topic_name" required value="{{ old('topic_name') }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        placeholder="e.g., Databases, Networks, Biology">
                    @error('topic_name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-8">
                    <label for="num_questions" class="block text-sm font-semibold text-gray-700 mb-2">Number of Items</label>
                    <select id="num_questions" name="num_questions" required
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                        <option value="">Select number</option>
                        @foreach([5,10,15,20,25,30] as $n)
                            <option value="{{ $n }}" {{ old('num_questions') == $n ? 'selected' : '' }}>{{ $n }} Items</option>
                        @endforeach
                    </select>
                    @error('num_questions')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
